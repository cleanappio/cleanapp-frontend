...
const CustomDashboardReport: React.FC<CustomDashboardReportProps> = ({
  ...
  const markAsFixed = async () => {
    ...
    try {
      const response = await reportProcessingApiClient.markFixed({
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      // Add marker
      L.circleMarker(
        [reportItem.report.latitude, reportItem.report.longitude],
        {
          radius: 8,
          color: "#ef4444",
          fillColor: "#ef4444",
      ...
      title.textContent =
        reportAnalysis?.title || `${t("report")} #${reportItem.report.seq}`;
      ...
      if (reportAnalysis?.brand_display_name || reportAnalysis?.brand_name) {
        ...
        brandName.textContent =
          reportAnalysis?.brand_display_name ||
          reportAnalysis?.brand_name ||
          "";
      ...
      if (reportAnalysis) {
        ...
        // Litter Probability
        if (reportAnalysis?.litter_probability !== undefined) {
        ...
        // Hazard Probability
        if (reportAnalysis?.hazard_probability !== undefined) {
      ...
      if (reportAnalysis) {
        ...
        if (reportAnalysis?.title) {
          const titleDiv = document.createElement("h4");
          titleDiv.textContent = reportAnalysis?.title;
        ...
        if (reportAnalysis?.description) {
          const descDiv = document.createElement("p");
          descDiv.textContent = reportAnalysis?.description;
      ...
      <div className="fixed inset-0 bg-white z-[2000] flex items-center justify-center">
        <div className="bg-red-50 border border-red-200 rounded-lg shadow-lg p-6 max-w-md text-center">
          ...
          <p className="text-red-700 mb-4">
            {t("mustBeLoggedInToViewReportDetails")}
          </p>
          <Link
      ...
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">
            {t("loading")} {t("report").toLowerCase()}...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-white z-[2000] overflow-y-auto lg:overflow-hidden">
      ...
      {/* Main Content */}
      <div
        className="pt-24 lg:pt-16 min-h-screen lg:h-full flex flex-col lg:flex-row overflow-y-auto lg:overflow-hidden"
        ref={contentRef}
      >
        {/* Image Section */}
