...
export function getColorByValue(value: number): string {
  ...
  if (clampedValue <= 0.25) {
    ...
    return `rgb(${Math.round(255 + ratio * 0)}, ${Math.round(255 - ratio * 128)}, ${Math.round(0 + ratio * 0)})`;
  } else if (clampedValue <= 0.75) {
    // Orange to Red (0.5 - 0.75)
    const ratio = (clampedValue - 0.5) / 0.25;
    return `rgb(${Math.round(255 + ratio * 0)}, ${Math.round(127 - ratio * 127)}, ${Math.round(0 + ratio * 0)})`;
  } else {
    // Red to Dark Red (0.75 - 1.0)
    const ratio = (clampedValue - 0.75) / 0.25;
    return `rgb(${Math.round(255 - ratio * 127)}, ${Math.round(0 + ratio * 0)}, ${Math.round(0 + ratio * 0)})`;
  }
}
